...
were during those logged runs (all on one LAN, or the edge laptop across town), so we treat the
5--15~ms round-trip in our mid-evaluation report as a descriptive figure, not one tied to this
log --- and the new campaign will record placement and network metadata with every
...
degradation actually costs, grounded in a real failure; (3) a validated cost-model-based placement
policy; (4) an open, reproducible testbed with all artifacts.
