...
in Qdrant), reciprocal-rank fusion, and LLM generation (Qwen3.8-27B, W4A16, served by
...
itself ended for operational reasons, not hardware: the edge laptop was not always available for
demonstrations, so we consolidated its duties into the gateway and the two-node system worked
fine.
...
0/15/40/80~ms RTT, applied bidirectionally so regimes shape every hop), and re-validate key
configurations over the genuine
...
regimes --- never the degradation study or the quality metrics. We target systems conferences:
\emph{IEEE ICDCS 2027} (submissions expected December 2026), with \emph{ACM HPDC 2027}
(deadline January 2027) as fallback; a journal extension (e.g.\ \emph{Future Generation Computer
Systems}) remains open for the longer version.
