...
roughly 538 passages per second on an RTX 3090. The system runs on two LAN machines over gRPC
(WireGuard): a gateway node (embedding, dense+sparse retrieval, fusion) on an Intel i7-10750H
...
edge as entry point and sparse-retrieval engine, across a metropolitan-fiber link treated as the
WAN leg.
...
edge-side sparse retrieval in parallel with transit of the query toward the LAN nodes:
$T_{\mathrm{retrieval}} = \max(T_{\mathrm{sparse}},\; T_{\mathrm{WAN}} + T_{\mathrm{dense}})$.
Two things undid this. First, the ``WAN'' was not much of one: the edge laptop sat at a home
across the city on metropolitan fiber, and our benchmarks put the round trip at only 5--15~ms
(~20~ms with WireGuard and protocol overhead) --- there was almost no transit latency to hide.
Second, and more fundamentally, parallel execution can save at most
$\min(T_{\mathrm{sparse}},\; T_{\mathrm{WAN}} + T_{\mathrm{dense}})$, and our 100-query logs from
the three-node run show sparse retrieval took just 0.6--10~ms --- too cheap to be worth
distributing --- while the split made the dense branch pay the extra 20~ms plus coordination
overhead. When the edge laptop then failed outright, we consolidated its duties into the gateway,
and the two-node system worked fine.
...
caching of retrieved knowledge (RAGCache~\cite{ragcache}, TurboRAG~\cite{turborag}), serving
configuration on a \emph{single} server (RAGO~\cite{rago}, METIS~\cite{metis}), index placement on
one device (EdgeRAG~\cite{edgerag}, VectorLiteRAG~\cite{vectorliterag}), or data distribution
...
(1) The first factorial measurement of stage-to-node placement for hybrid RAG on commodity
