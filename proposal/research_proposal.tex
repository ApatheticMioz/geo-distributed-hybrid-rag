...
edge as entry point and sparse-retrieval engine across a WAN link.
...
edge-side sparse retrieval in parallel with WAN transit of the query toward the LAN:
$T_{\mathrm{retrieval}} = \max(T_{\mathrm{sparse}},\; T_{\mathrm{WAN}} + T_{\mathrm{dense}})$.
Measurements did not bear this out. On our campus LAN the round-trip time is only 5--15~ms, so the
latency we hoped to hide was small next to retrieval and generation costs, and the third node added
coordination overhead. When the edge laptop failed, we consolidated its duties into the gateway,
and the two-node system worked fine.

That failure motivates this proposal. We learned that a fixed placement choice can silently cost
performance, and that topology change (node loss, consolidation) is a normal event in real
deployments --- yet neither we nor, as far as we can find, the literature can answer the natural
questions: \emph{where should each stage of a hybrid RAG pipeline run, under which network
...
multi-node hybrid RAG deployment --- sparse retrieval, dense retrieval, fusion, document hydration,
and generation can each sit on a different node, and the right answer plausibly changes with RTT,
bandwidth, and node capability. Topology failure (what happens to latency and answer quality when a
node drops out and roles consolidate) appears entirely unmeasured for RAG.
