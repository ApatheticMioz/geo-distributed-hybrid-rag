...
Let the retrieval phase be $\max(\Tsparse,\Tdense)$ (parallel legs) and let
hydration cost $\Thydrate$ for the top passages. The four topologies are:
\begin{align}
\text{P0:}\quad \Tttft^{(0)} &= \max(\Tsparse,\Tdense)+\Tfusion+\Thydrate+\Tprefill,
\label{eq:p0}\\
\text{P2:}\quad \Tttft^{(2)} &= \max(\Tsparse,\Tdense)+\Tfusion
+\Twan(p_{\mathrm{id}})+\Thydrate+\Tprefill,
\label{eq:p2}\\
\text{P3:}\quad \Tttft^{(3)} &= \max(\Tsparse,\Tdense)+\Tfusion+\Thydrate
+\Twan(p_{\mathrm{text}})+\Tprefill,
\label{eq:p3}
\end{align}
...
\begin{equation}
\Tttft^{(2,\mathrm{SPHP})} \;=\;
h\,\max(t_{f},t_{p}) \;+\; (1-h)\left(\Tttft^{(2)}+T_{\mathrm{WAN,recon}}\right).
\label{eq:sphp}
\end{equation}
